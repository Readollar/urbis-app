...
export default function HomesScreen() {
    ...
    return (
      <View className="flex-1 bg-white">
        ...
        <View className="px-6 pt-6">
          <Text className="text-3xl font-bold text-blue-900 mb-2">Host on Urbis</Text>
          <Text className="text-sm text-gray-500 font-medium leading-5 mb-12">Turn your property into extra income. Join thousands of verified hosts across Nigeria.</Text>
          <View className="bg-blue-50 rounded-3xl p-8 items-center border border-blue-100">
            <View className="bg-white w-20 h-20 rounded-full justify-center items-center mb-6 shadow-sm"><Ionicons name="home" size={36} color="#2563EB" /></View>
            <Text className="text-xl font-bold text-blue-900 text-center mb-3">List your space</Text>
            <Text className="text-gray-600 text-center mb-8 leading-6">Whether it's a duplex in Lekki, an apartment in Ibadan, or a short-let in Abuja, you can list it here.</Text>
            <TouchableOpacity className="bg-blue-600 w-full py-4 rounded-full shadow-md shadow-blue-500/30" onPress={() => router.push('/add-property')}>
  ...
  return (
    <View className="flex-1 bg-white">
      <TopBar />
      <View className="px-6 pt-6 mb-6 flex-row justify-between items-end">
        <View>
          <Text className="text-3xl font-bold text-blue-900 mb-1">Host Dashboard</Text>
          <Text className="text-sm text-gray-500 font-medium">Manage your portfolio.</Text>
        </View>
        <TouchableOpacity className="bg-blue-600 w-10 h-10 rounded-full justify-center items-center shadow-sm" onPress={() => router.push('/add-property')}>
      ...
      <View className="px-6 mb-6 flex-row gap-3">
        <View className="flex-1 bg-gray-50 p-4 rounded-2xl border border-gray-100">
          ...
          <Text className="text-3xl font-black text-blue-900">{myProperties.length}</Text>
        </View>
      ...
      {myProperties.length === 0 ? (
        <View className="flex-1 justify-center items-center px-8 pt-10">
        ...
        <ScrollView showsVerticalScrollIndicator={false} className="px-6" refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor="#2563EB" />}>
          {myProperties.map((property) => (
            <TouchableOpacity 
              key={property.id} 
              activeOpacity={0.8} 
              onPress={() => router.push(`/property/${property.id}`)} 
              // Removed w-full, let flex naturally handle the stretch. Added p-4 for breathing room.
            ...
            >
              ...
              <View className="flex-1">
                ...
                <Text className="text-blue-600 font-bold mb-1">₦{property.price.toLocaleString()}</Text>
